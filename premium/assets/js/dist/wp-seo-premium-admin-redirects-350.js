...
(function ($) {
  ...
  */
	RedirectQuickEdit.prototype.setup = function (row, row_cells) {
		this.row = row;
		this.quick_edit_row = $(templateQuickEdit({
  ...
  */
   ...
   */
		var reset_ignore = function reset_ignore() {
			ignore = false;
			last_action = null;
		};
   ...
   */
		this.create_redirect_row = function (old_url, new_url, redirect_type, redirectInfo) {
			...
			if (!redirectInfo.isTargetRelative || '' === new_url || '/' === new_url) {
			...
			var tr = $('<tr>').append($('<th>').addClass('check-column').attr('role', 'row').append($('<input>').attr('name', 'wpseo_redirects_bulk_delete[]').attr('type', 'checkbox').val(_.escape(old_url)))).append($('<td>').append($('<div>').addClass('val type').html(_.escape(redirect_type))).append($('<div>').addClass('row-actions').append($('<span>').addClass('edit').append($('<a>').attr('href', 'javascript:;').html('Edit')).append(' | ')).append($('<span>').addClass('trash').append($('<a>').attr('href', 'javascript:;').html('Delete'))))).append($('<td>').addClass('column-old').append($('<div>').addClass('val').html(_.escape(old_url)))).append($('<td>').addClass('column-new').append($('<div>').addClass(targetClasses.join(' ')).html(_.escape(new_url))));
   ...
   */
		this.setup = function () {
			...
			$('.wpseo-new-redirect-form').on('click', 'a.button-primary', function () {
				last_action = function last_action() {
					that.add_redirect();
				};
			...
			$('.wp-list-table').on('click', '.edit', function (evt) {

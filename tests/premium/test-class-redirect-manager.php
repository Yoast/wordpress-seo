...
class WPSEO_Redirect_Manager_Test extends WPSEO_UnitTestCase {
	...
	public function setUp() {
		...
		$this->class_instance->create_redirect( new WPSEO_Redirect( 'regex', 'newwest', 301, 'regex' ) );
	}
	...
	public function test_update_redirect() {
		...
		$updated_redirect = new WPSEO_Redirect( 'redirect_update', 'updated_redirect', 301 );

		$this->assertTrue( $this->class_instance->update_redirect( $redirect, $updated_redirect ) );

		$this->assertEquals( 'redirect_update', $updated_redirect->get_origin() );
		$this->assertEquals( 'updated_redirect', $updated_redirect->get_target() );
		$this->assertEquals( '301', $updated_redirect->get_type() );

		// Update a non existing redirect.
		$non_existing_redirect = new WPSEO_Redirect( 'i-do-not-exists', 'updated_redirect', 301 );
	...
	public function test_delete_redirect() {
		...
		$redirect = new WPSEO_Redirect( 'delete_redirect', 'deleted_redirect' );

		// First of all create a redirect.
		$this->class_instance->create_redirect( $redirect, 301 );

		// Remove the redirect.
		$this->assertTrue( $this->class_instance->delete_redirects( array( $redirect ) ) );

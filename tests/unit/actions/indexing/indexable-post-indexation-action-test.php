...
class Indexable_Post_Indexation_Action_Test extends TestCase {
	...
	/**
	 * Tests the index method that no transients are deleted when there are no indexables found.
	 *
	 * @covers ::__construct
	 * @covers ::index
	 * @covers ::get_limit
	 * @covers ::get_post_types
	 */
	public function test_index_no_indexables_found() {
		$expected_query = "
			SELECT P.ID
			FROM wp_posts AS P
			WHERE P.post_type IN (%s)
		...
		Functions\expect( 'delete_transient' )->with( 'wpseo_total_unindexed_posts' )->never();
		Functions\expect( 'delete_transient' )->with( 'wpseo_total_unindexed_posts_limited' )->never();

...
function getFeedItems( parsed, nsResolver, maxItems ) {
	let snapshots = getXPathSnapshots( '/rss/channel/item', parsed );
	...
	for ( let i = 0; i < count; i++ ) {
		let snapshot = snapshots.snapshotItem( i );
...
function getXPathText( xpath, document, context = null, nsResolver = null ) {
	let result = document.evaluate( xpath , ( context || document ), nsResolver, XPathResult.STRING_TYPE, null );

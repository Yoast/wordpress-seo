...
export const analysisAsyncActions = reduce(ASYNC_ACTIONS, (actions, action) => ({ ...actions,
  [action]: `${ANALYZE_ACTION_NAME}/${action}`
}), {});
/**
...
export function* analyze() {
  yield {
    type: analysisAsyncActions.request
  };
  ...
  try {
    ...
    return {
      type: analysisAsyncActions.success,
      payload: processedResults
    };
  } catch (error) {
    return {
      type: analysisAsyncActions.error,
      payload: error
    };
  }
}
...
const resultsSlice = createSlice({
  ...
  reducers: {
    ...
    updateMarkerStatus: (state, _ref2) => {
      let {
        payload
      } = _ref2;
  ...
  extraReducers: builder => {
    ...
    builder.addCase(analysisAsyncActions.success, (state, _ref3) => {
      let {
        payload
      } = _ref3;
      state.status = ASYNC_STATUS.SUCCESS; // Update SEO results state for each keyphrase
    ...
    builder.addCase(analysisAsyncActions.error, (state, _ref4) => {
      let {
        payload
      } = _ref4;
...
export const resultsSelectors = {
  selectSeoScore: function (state) {
    let id = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : FOCUS_KEYPHRASE_ID;
    return get(state, `analysis.results.seo.${id}.score`);
  },
  selectSeoResults: function (state) {
    let id = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : FOCUS_KEYPHRASE_ID;
    return get(state, `analysis.results.seo.${id}.results`);
  },

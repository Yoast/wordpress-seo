 ...
 * @returns {*} The table and thead.
 */
const Table = ( { children, header } ) => {
	return (
		<div className="yst-overflow-x-scroll yst-border yst-border-gray-200 yst-shadow-sm yst-rounded-lg">
...
const ListTable = ( { children, header, status, label, columns } ) => {
	...
	if ( status === "loading" ) {
		return (
			<Table header={ header }>
				<PlaceholderRows columnCount={ columns.length - 1 } />
			</Table>
		);
			...
			<Table header={ header }>
				<tr>
					<td colSpan={ 100 } className="yst-w-full yst-h-60 yst-text-center yst-text-red-600">
						{ /* translators: %s is replaced by the contentType label. */ }
						{ sprintf( __( "Oops, something went wrong while retrieving your %s. Please refresh the page. ", "admin-ui" ), contentTypeLabel ) }
					</td>
				</tr>
			</Table>
		);
...
const EmptyRow = ( { label } ) => {
	...
	return <tr>
		<td colSpan={ 100 } className="yst-w-full yst-h-56 yst-text-center">
			{ /* translators: %s is replaced by the contentType label. */ }
			{ sprintf( __( "There are no %s to show. ", "admin-ui" ), contentTypeLabel ) }
		</td>
	</tr>;
};
 ...
 * @returns {*} An icon or label, either with or without a button for sorting.
 */
 ...
 * @returns {*} The Table Heading component.
 */

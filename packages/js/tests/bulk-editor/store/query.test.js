...
describe( "query slice", () => {
	it( "defaults to an empty search on the first page", () => {
		expect( createInitialQueryState() ).toEqual( { search: "", page: 1 } );
	} );
	...
	it( "selects the search term, page and query from the store state", () => {
		const state = { query: { search: "seo", page: 2 } };

		expect( querySelectors.selectSearch( state ) ).toBe( "seo" );
		expect( querySelectors.selectPage( state ) ).toBe( 2 );
		expect( querySelectors.selectQuery( state ) ).toEqual( { search: "seo", page: 2 } );
	} );
	...
	it( "falls back to defaults when the state is missing", () => {
		...
		expect( querySelectors.selectQuery( {} ) ).toEqual( { search: "", page: 1 } );
	} );
} );

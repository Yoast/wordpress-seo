...
import { SearchNoResultsContent } from "./search-no-results-content";
import { SearchResultLabel } from "./search-result-label";
import { useParsedUserAgent } from "../../hooks";
								...
								<div key={ groupedItems?.[ 0 ]?.route || `group-${ index }` } role="presentation">
									...
									<div role="presentation">
										{ map( groupedItems, ( item ) => (
											...
											>
												<SearchResultLabel { ...item } keyFilterPattern={ keyFilterPattern } />
											</Combobox.Option>
										) ) }
									</div>
								</div>

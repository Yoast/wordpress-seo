...
export const useTitleOrDescriptionLengthProgress = ( { editType, title, description } ) => {
	...
	const isCornerstone = useSelect( select => select( STORE_NAME_EDITOR.free ).isCornerstoneContent(), [] );
	const isTaxonomy = useSelect( select => select( STORE_NAME_EDITOR.free ).getIsTerm(), [] );

...
export default function AIButton( state = INITIAL_STATE, action ) {
	switch ( action.type ) {
		case SET_ACTIVE_AI_FIXES_BUTTON: {
			// When the active button is set to null, store the previous active button as the focus target.
			const focusAIButton = action.activeAIButton === null && state.activeAIButton !== null
				? state.activeAIButton
				: state.focusAIButton;
			return {
				...state,

 ...
 *
 * @param {string} title The title of the content suggestion block.
 * @param {string[]} suggestions The content suggestions to display.
 *
 * @returns {JSX.Element} The content suggestion block component.
 */
export const ContentSuggestionBlock = ( { title, suggestions } ) => {
	const svgAriaProps = useSvgAria();
	return (
		<div className="yst-bg-slate-50 yst-rounded-sm yst-shadow-sm yst-border yst-border-solid yst-border-slate-200 yst-p-4">
			...
			<div>
				<div className="yst-flex yst-justify-start yst-items-center yst-gap-2 yst-mb-1">
					...
					<h3 className="yst-text-slate-800 yst-text-sm yst-leading-5 yst-font-medium yst-m-0">{ title }</h3>
				</div>
				{ suggestions.length > 0 &&
				<ul className="yst-text-sm yst-text-slate-600 yst-font-normal yst-my-0 yst-ps-5 yst-leading-5 yst-list-none">
					{ suggestions.map( ( suggestion, index ) => (
						<li className="yst-flex yst-justify-start yst-gap-2" key={ `${index}-${title}` }>

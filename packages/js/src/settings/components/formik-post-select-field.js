...
const FormikPostSelectField = ( { name, id, className = "", ...props } ) => {
	const posts = useSelectSettings( "selectPostsWith");
	...
	useEffect( () => {
		// Get initial options.
		console.log(status);
		console.log(ASYNC_ACTION_STATUS);
		debouncedFetchPosts( "" );
	}, [] );
					...
					<>
						...
						{ canCreatePosts && (
							<li className="yst-sticky yst-inset-x-0 yst-bottom-0 yst-group">
								...
								>
									<DocumentAddIcon className="yst-w-5 yst-h-5 yst-text-slate-400 group-hover:yst-text-white" />

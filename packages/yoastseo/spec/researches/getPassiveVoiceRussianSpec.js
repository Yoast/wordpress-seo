...
describe( "detecting passive voice in sentences", function() {
	...
	it( "returns passive voice in Russian", function() {
		// Passive: исследован
		const paper = new Paper( "Экспериментально исследован спектр ИК-фотопроводимости, возникающей после предварительного освещения образца F-светом.", { locale: "ru_RU" } );
	...
	it( "returns passive voice in Russian", function() {
		...
		const paper = new Paper( "В этой нише представлены, по сути, две основные компании ― Tyan и Supermicro.", { locale: "ru_RU" } );
		expect( passiveVoice( paper ).passives.length ).toBe( 1 );
	} );
	...
	it( "returns passive voice in Russian", function() {
		// Passive: сказано
		const paper = new Paper( "И этим всё сказано: продукция Supermicro считается более надёжной, но и стоит подороже.", { locale: "ru_RU" } );
		expect( passiveVoice( paper ).passives.length ).toBe( 1 );
	} );

	it( "returns active voice in Russian", function() {
		const paper = new Paper( "Supermicro выполняет свои материнские платы на основе наборов микросхем Intel, а также ServerWorks.", { locale: "ru_RU" } );
		expect( passiveVoice( paper ).passives.length ).toBe( 0 );
	} );

	it( "returns passive voice in Russian", function() {
		// Passive: выполнены
		const paper = new Paper( "Материнские платы Supermicro выполнены на основе наборов микросхем Intel, а также ServerWorks.", { locale: "ru_RU" } );
		expect( passiveVoice( paper ).passives.length ).toBe( 1 );
	} );
	...
	it( "returns passive voice in Russian", function() {
		// Passive: убеждена
		const paper = new Paper( "Я мечтал в детстве о велике, но моя мама мне велик не покупала оттого, что была убеждена, будто на велике я непременно попаду под машину.", { locale: "ru_RU" } );
		expect( passiveVoice( paper ).passives.length ).toBe( 1 );
	} );

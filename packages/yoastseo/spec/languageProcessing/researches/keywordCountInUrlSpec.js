import urlKeyword from "../../../src/languageProcessing/researches/keywordCountInUrl.js";
...
describe( "test to check url for keyword", function() {
	...
	it( "works with multi-word keyphrases", function() {
		const paper = new Paper( "", { url: "key-word-in-slug-how-wonderful", keyword: "key words within slugs are " +
	...
	it( "works with multi-word keyphrases", function() {
		const paper = new Paper( "", { url: "how-wonderful-key-word-in-slug", keyword: "key words within slugs are " +
				"always wonderful", locale: "en_EN" } );
		const researcher = new EnglishResearcher( paper );
		researcher.addResearchData( "morphology", morphologyData );
		expect( urlKeyword( paper, researcher ) ).toEqual( { keyphraseLength: 4, percentWordMatches: 100 } );
	...
	it( "correctly deals with multiple matches with multi-word keyphrases", function() {
		const paper = new Paper( "", { url: "how-wonderful-wonderful-a-wonderful-slug", keyword: "key words within slugs " +

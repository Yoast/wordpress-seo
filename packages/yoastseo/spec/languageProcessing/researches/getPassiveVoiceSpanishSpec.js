...
describe( "detecting passive voice in sentences", function() {
	...
	it( "returns passive voice (pretérito)", function() {
		// Passive: fue comida.
		const paper = new Paper( "Una manzana fue comida por mí.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 1 );
	} );

	it( "returns active voice (present perfect)", function() {
		const paper = new Paper( "Yo he comido una manzana.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );

	it( "returns passive voice (present perfect)", function() {
		// Passive: ha sido comida.
		const paper = new Paper( "Una manzana ha sido comida por mí.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 1 );
	} );

	it( "returns active voice (imperfect)", function() {
		const paper = new Paper( "Yo comía una manzana.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );

	it( "returns passive voice (imperfect)", function() {
		// Passive: era comida.
		const paper = new Paper( "Una manzana era comida por mí.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 1 );
	} );

	it( "returns active voice (pluperfect)", function() {
		const paper = new Paper( "Yo había comido una manzana.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );

	it( "returns passive voice (pluperfect)", function() {
		// Passive: había sido comida.
		const paper = new Paper( "Una manzana había sido comida por mí.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 1 );
	} );

	it( "returns active voice (conditional)", function() {
		const paper = new Paper( "Yo comería una manzana.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );

	it( "returns passive voice (conditional)", function() {
		// Passive: sería comida.
		const paper = new Paper( "Una manzana sería comida por mí.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 1 );
	} );

	it( "returns active voice (present subjunctive)", function() {
		const paper = new Paper( "Yo coma una manzana.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );

	it( "returns passive voice (present subjunctive)", function() {
		// Passive: sea comida.
		const paper = new Paper( "Una manzana sea comida por mí.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 1 );
	} );

	it( "returns active voice (infinitive)", function() {
		const paper = new Paper( "Yo debo comer una manzana.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );

	it( "returns passive voice (infinitive)", function() {
		// Passive: ser comida.
		const paper = new Paper( "Una manzana debe ser comida por mí.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 1 );
	} );
	...
	it( "doesn't return passive voice if an auxiliary is followed by a word from the followingAuxiliaryExceptionWords list", function() {
		...
		const paper = new Paper( "Es el capítulo preferido de constios miembros del equipo de producción.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );
	...
	it( "doesn't return passive voice if a participle is marked as non-passive by a precedence exception (directly preceding).", function() {
		...
		const paper = new Paper( "Fue un sentido monumental y grandilocuente.", { locale: "es_ES" } );
		expect( passiveVoice( paper, new Researcher( paper ) ).passives.length ).toBe( 0 );
	} );
} );

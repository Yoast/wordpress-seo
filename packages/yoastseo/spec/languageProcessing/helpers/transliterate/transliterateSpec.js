...
describe( "A test for removing special characters from text.", function() {
	...
	it( "returns a Silesian string without special characters.", function() {
		expect( transliteration( "ÃĆŁNŎŌÔÕŚŹŻŽŮČŘŠãćłnŏōôõśźżžůčřš", "szl" ) ).toBe( "ACUNOOOOSZZZUCzRzSzacunooooszzzuczrzsz" );
	} );
	it( "returns a Tahitian string without special characters.", function() {
		expect( transliteration( "’'‘āâàĀÂÀïîìÏÎÌēêéĒÊÉūûúŪÛÚçÇòôōÒÔŌ", "tah" ) ).toBe( "aaaAAAiiiIIIeeeEEEuuuUUUcCoooOOO" );
	} );

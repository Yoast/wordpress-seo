...
const functionWords = [ "and", "the", "them" ];
...
describe( "A test for building stems for an array of words", function() {
	it( "returns an empty array if the input keyphrase is undefined", function() {
		let keyphrase;
		const forms = buildStems( keyphrase, baseStemmer, functionWords );
	...
	it( "returns all (content) words if there is no morphological analyzer for this language yet", function() {
		const forms = buildStems( "Je ne vais pas rire", baseStemmer, functionWords );
		expect( forms ).toEqual(
			new TopicPhrase(
...
// 	} );
//
// 	it( "returns all (content) words if there is no morphological analyzer for this language (i.e., Free situation) " +
// 		"and takes care of apostrophe variations", function() {

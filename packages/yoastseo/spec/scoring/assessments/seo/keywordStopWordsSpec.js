...
describe( "A stop word in keyword assessment", function() {
	it( "assesses no stop words in the keyword", function() {
		const mockPaper = new Paper( "sample text without stopwords", {
			keyword: "sample",
		} );

		const assessment = stopWordsInKeywordAssessment.getResult( mockPaper, Factory.buildMockResearcher( [] ), i18n );
	...
	it( "assesses one stop word in the keyword", function() {
		...
		const assessment = stopWordsInKeywordAssessment.getResult( mockPaper, Factory.buildMockResearcher( [ "about" ] ), i18n );
		expect( assessment.getScore() ).toEqual( 0 );
		expect( assessment.hasScore() ).toEqual( true );
		expect( assessment.getText() ).toEqual( "<a href='https://yoa.st/34b' target='_blank'>Stopwords</a>:" +
			" The keyphrase contains stop words. This may or may not be wise depending on the circumstances." +
			" <a href='https://yoa.st/34c' target='_blank'>Learn more about stop words</a>." );
	} );
	...
	it( "assesses multiple stop words in the keyword", function() {
		...
		const assessment = stopWordsInKeywordAssessment.getResult( mockPaper, Factory.buildMockResearcher( [ "about", "before" ] ), i18n );
		expect( assessment.getScore() ).toEqual( 0 );
		expect( assessment.getText() ).toEqual( "<a href='https://yoa.st/34b' target='_blank'>Stopwords</a>: The keyphrase contains stop words." +
			" This may or may not be wise depending on the circumstances. <a href='https://yoa.st/34c' target='_blank'>Learn more" +
			" about stop words</a>." );
	} );
} );
...
describe( "Checks if the assessment is applicable", function() {
	...
	it( "returns true for isApplicable for an English paper with keyword.", function() {
		const paper = new Paper( "", { locale: "en_EN", keyword: "keyword" } );
	...
	it( "returns false for isApplicable for an Dutch paper with keyword.", function() {
		const paper = new Paper( "", { locale: "nl_NL", keyword: "keyword" } );
		expect( stopWordsInKeywordAssessment.isApplicable( paper ) ).toBe( false );
	} );
} );

import recommendedKeywordCount from "../../../../src/scoring/helpers/assessments/recommendedKeywordCount.js";
...
describe( "Test for getting the recommended keyword count for a text", function() {
	it( "returns the maximum recommended keyword count for a text with 300 words and a 1-word keyphrase", function() {
		...
		expect( recommendedKeywordCount( paper, 1, maxRecommendedDensity, "max" ) ).toBe( 8 );
	} );

	it( "returns the maximum recommended keyword count for a text with 300 words and a 1-word keyphrase, " +
		...
		const paper = new Paper( "<p>" + text + "</p>" );
		buildTree( paper, defaultResearcher );
		expect( recommendedKeywordCount( paper, 1, maxRecommendedDensity, "max", japaneseWordHelper ) ).toBe( 8 );
	} );
	...
	it( "returns the maximum recommended keyword count for a text with 300 words and a multi-word keyphrase", function() {
		...
		expect( recommendedKeywordCount( paper, 2, maxRecommendedDensity, "max" ) ).toBe( 6 );
	} );
	...
	it( "returns the minimum recommended keyword count for a text with 1200 words and a 1-word keyphrase", function() {
		...
		expect( recommendedKeywordCount( paper, 1, minRecommendedDensity, "min" ) ).toBe( 6 );
	} );
	...
	it( "returns 2 as the default recommended minimum keyword count if the text is very short", function() {
		...
		expect( recommendedKeywordCount( paper, 1, minRecommendedDensity, "min" ) ).toBe( 2 );
	} );
	...
	it( "returns 0 when the word count of the text is 0", function() {
		...
		expect( recommendedKeywordCount( paper, 1, maxRecommendedDensity, "max" ) ).toBe( 0 );
	} );
	...
	it( "returns the maximum recommended keyword count when the maxOrMin variable is not explicitly defined", function() {
		...
		expect( recommendedKeywordCount( paper, 1, maxRecommendedDensity ) ).toBe( 8 );
	} );

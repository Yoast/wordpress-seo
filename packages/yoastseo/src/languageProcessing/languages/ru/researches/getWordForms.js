...
export default function( paper, researcher ) {
	...
	const allWordsFromPaper = getAllWordsFromPaper( paper ).map( word => word.toLocaleLowerCase( "ru" ) );
	const keyphrase = paper.getKeyword().toLocaleLowerCase( "ru" ).trim();
	const synonyms = parseSynonyms( paper.getSynonyms().toLocaleLowerCase( "ru" ).trim() );

	// In Russian we have function words, a stemmer and morphology data.

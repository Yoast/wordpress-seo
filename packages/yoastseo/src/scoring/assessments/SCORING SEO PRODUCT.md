...
**What it does**: Checks if all assessed images have alt attributes. By default it assesses the images in the text; a platform can scope all image assessments to the product's own images (featured, gallery, variations) instead, by providing the Paper's `productImages` attribute — an empty array counts as a product without images.

...
if [ $? -eq 1 ]; then
	...
	if [ $? -eq 1 ]; then
		wp-env run tests-cli wp plugin install "https://github.com/Yoast/wordpress-seo/blob/trunk/packages/e2e-tests/data/yoast-simple-custom-post-type.zip?raw=true"

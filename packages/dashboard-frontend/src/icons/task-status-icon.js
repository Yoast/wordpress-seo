...
export const TaskStatusIcon = ( { isCompleted, isLoading } ) => {
	if ( isLoading ) {
		return <EllipseWithInnerDot className="yst-w-6 yst-text-slate-200 yst-shrink-0" role="img" aria-hidden="false" aria-label={ __( "Task loading", "wordpress-seo" ) } />;
	}
	if ( isCompleted ) {
	...
	return <EllipseWithInnerDot className="yst-w-6 yst-text-primary-500 yst-shrink-0" role="img" aria-hidden="false" aria-label={ __( "Task not completed", "wordpress-seo" ) } />;
};

...
const transformRelatedKeyprases = relatedKeyphrases => mapValues(relatedKeyphrases, // 'Keyphrase' is renamed to 'keyword', 'synonyms' should stay as is.
_ref => {
  let {
    keyphrase: keyword,
    synonyms
...
const serializeSeoResults = seoResults => mapValues(seoResults, // Keep the score as is, but serialize the assessment results.
_ref2 => {
  let {
    score,
    results
...
const renameFocusKeyphraseKey = seoResults => mapKeys(seoResults, (_, key) => key === "" ? FOCUS_KEYPHRASE_ID : key);
/**
...
const analyzePaper = async (worker, paper, relatedKeyphrases) => {
  ...
  const results = await worker.analyzeRelatedKeywords(paper, transformRelatedKeyprases(relatedKeyphrases));
...
const runResearches = async (researches, worker, paper) => {
  const results = await Promise.all(researches.map(research => worker.runResearch(research, paper)));
...
const createAnalyzeFunction = (worker, configuration) => {
   ...
   */
  return async function (data, keyphrases) {
    let config = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    const {
      [FOCUS_KEYPHRASE_ID]: focusKeyphrase,
      ...relatedKeyphrases
    } = keyphrases;
    const paper = createPaper(data, focusKeyphrase, configuration);
    const [analysisResults, researchResults] = await Promise.all([analyzePaper(worker, paper, relatedKeyphrases), runResearches(config.researches, worker, paper)]);

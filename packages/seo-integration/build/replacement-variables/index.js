...
const registerApplyReplacementVariables = applyers => {
   ...
   */
  const applyReplacementVariables = (paper, _ref) => {
    let {
      config: {
        analysisType
      }
    } = _ref;
    return mapValues(paper, get(applyers, analysisType, identity));
  };

  addFilter("yoast.seoStore.analysis.preparePaper", "yoast/seoIntegration/applyReplacementVariables", applyReplacementVariables, 10);
...
const createAnalysisTypeReplacementVariables = configurations => {
  ...
  const analysisTypeReplacementVariables = mapValues(filteredConfigurations, (analysisTypeConfigurations, analysisType) => {
    const filteredAnalysisTypeConfigurations = applyFilters(`yoast.seoIntegration.replacementVariables.${analysisType}.configurations`, analysisTypeConfigurations, {
      analysisType
    });

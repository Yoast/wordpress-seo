...
const augmentedIcons = Object.assign( icons, iconToAdd );
...
export default function SvgIconsWrapper() {
		...
		return transform( iconSet, ( result, value, key ) => {
			...
			return result.push(
				<SingleSvgContainer key={ key }>
					<SvgIcon iconSet={ iconSet } icon={ key } size="40px" color={ color } />
	...
	return (
		<SvgIconsContainer>
			...
			<h2>Single icon from default icon set</h2>
			<SvgIcon icon="desktop" size="40px" />
			<h2>Single icon from alternative icon set</h2>
			<SvgIcon iconSet={ alternativeIcons } icon="home" size="40px" />
		</SvgIconsContainer>
	);
}

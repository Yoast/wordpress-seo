...
export function initialize( args ) {
	...
	if ( has( wp, "hooks.addFilter" ) && has( wp, "editPost.registerSidebar" ) ) {
		const { Panel, PanelBody, IconButton } = wp.components;
		const Icon = () => (
			<svg fill="#333" version="1.1" id="Layer_2_1_" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 841.9 595.3" enableBackground="new 0 0 841.9 595.3" xmlSpace="preserve" width="24px" height="24px">
				<g>
		...
		const AnalysisMenuComponent = () => {
			return (
				<IconButton
					...
					onClick={ () => wp.editPost.activateSidebar( "yoast/analysis" ) }>
					{ "Yoast SEO" }
				</IconButton>
			);
		};
		...
		wp.hooks.addFilter(
			...
			( items ) => [ ...items, <AnalysisMenuComponent/> ],
		);

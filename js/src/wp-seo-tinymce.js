...
function getContentTinyMce( content_id ) {
	if ( content_id == 0 || content_id == null || content_id === '' ) {
		console.warn( 'tinymceHelper content_id is undefined' )
	}
...
function isTinyMCEAvailable( editorID ) {
	if ( editorID == 0 || editorID == null || editorID === '' ) {
		console.warn( 'tinymceHelper content_id is undefined' )
	}
	if ( ! isTinyMCELoaded() ) {
		return false;
...
function tinyMceEventBinder( app, tmceId ) {
	if ( tmceId == 0 || tmceId == null || tmceId === '' ) {
		console.warn( 'tinymceHelper content_id is undefined' )
	}

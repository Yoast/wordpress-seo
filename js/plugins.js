...
YoastSEO.Plugins.prototype.register = function( pluginName, options ) {
	if ( ! ( typeof pluginName === 'string' && typeof status === 'string' ) ) {
		console.error('Failed to register plugin. Expected both parameters to be strings.');
		return false;
	}
...
YoastSEO.Plugins.prototype.error = function( name ) {
 ...
 * @param priority {number} (optional) Used to specify the order in which the callables associated with a particular filter are called. Lower numbers correspond with earlier execution.
 * @returns {boolean}
 */
...
YoastSEO.plugins = new YoastSEO.Plugins

/**
 * Poller to handle loading of plugins. Plugins can register with our app to let us know they are going to hook into our Javascript. They are allowed 5 seconds of pre-loading time to fetch all the data they need to be able to perform their data modifications.
 * We will only apply data modifications from plugins that have declared ready within the pre-loading time in order to safeguard UX and data integrity.
 *
...
YoastSEO.Plugins.prototype._pollLoadingPlugins = function (pollTime) {
	// set pollTime to 0 if undefined.

...
class Sqlite3AdapterTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function test_select_one()
    {
        ...
        $this->assertEquals(\true, \is_array($result));
    ...
    public function test_add_new_column()
    {
        ...
        $this->assertEquals(array('name' => 'name', 'type' => 'varchar(10)', 'field' => 'name', 'null' => 1, 'default' => \null), $this->adapter->column_info('test', 'name'));
    }
    ...
    public function test_create_schema_version_table()
    {
        ...
        if ($this->adapter->table_exists(\YOASTSEO_VENDOR__RUCKUSING_TS_SCHEMA_TBL_NAME, \true)) {
            $this->adapter->drop_table(\YOASTSEO_VENDOR__RUCKUSING_TS_SCHEMA_TBL_NAME);
        }
        $this->adapter->create_schema_version_table();
        $this->assertEquals(\true, $this->adapter->table_exists(\YOASTSEO_VENDOR__RUCKUSING_TS_SCHEMA_TBL_NAME, \true));
    }
    public function test_can_dump_schema()
    {
        ...
        $this->adapter->execute_ddl("CREATE INDEX idx_animals_on_name ON animals(name)");
        $file = \YOASTSEO_VENDOR__RUCKUSING_BASE . '/tests/logs/schema.txt';
    ...
    public function test_ensure_table_does_exist()
    {
        ...
        $t1 = new \YoastSEO_Vendor\Ruckusing_Adapter_Sqlite3_TableDefinition($this->adapter, "users");
    ...
    {
        $this->drop_table('users');
        $t1 = new \YoastSEO_Vendor\Ruckusing_Adapter_Sqlite3_TableDefinition($this->adapter, "users", array('id' => \true));

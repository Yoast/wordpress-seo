...
class Elementor implements Integration_Interface {
	...
	public function save_postdata() {
		...
		$no_slug_yet = $post->post_name === '' && $slug === '';
		if ( $no_slug_yet || $post->post_name !== $slug ) {
			...
			if ( $no_slug_yet ) {
				/*
				 * There was no slug yet: clean the cache.
				 * So when a new slug was generated by WP, we will receive it.
				 */
				\clean_post_cache( $post_id );
			}
			$post = \get_post( $post_id );

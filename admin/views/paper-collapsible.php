...
<div
	...
	if ( ! empty( $title ) ) {
		...
		if ( ! empty( $collapsible ) ) {
			...
			printf(
				'<h2 class="%1$s"><div%2$s type="button" class="toggleable-container-trigger" aria-expanded="%3$s">%4$s%5$s <span class="toggleable-container-icon dashicons %6$s" aria-hidden="true"></span></div></h2>',

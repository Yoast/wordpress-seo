...
class Yoast_View_Utils {
	...
	public function show_post_type_settings( $post_type ) {
		...
		$this->form->index_switch( 'noindex-' . $post_type->name, $post_type->labels->name );
		...
		$this->form->show_hide_switch( 'showdate-' . $post_type->name, __( 'Date in Snippet Preview', 'wordpress-seo' ) );
		/* translators: %1$s expands to Yoast SEO */
		$this->form->show_hide_switch( 'hideeditbox-' . $post_type->name, sprintf( __( '%1$s Meta Box', 'wordpress-seo' ), 'Yoast SEO' ) );
	}
}

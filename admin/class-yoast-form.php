...
class Yoast_Form {
	...
	public function checkbox( $var, $label, $label_left = false, $attr = [] ) {
		...
		$disabled_attribute = disabled(
			$this->is_control_disabled( $var ) || ( isset( $attr['disabled'] ) && $attr['disabled'] ),
			true,
			false
		);

		// phpcs:ignore WordPress.Security.EscapeOutput -- Reason: $disabled_attribute is generated by WordPress and all other output is properly escaped.
	...
	public function media_input( $var, $label, $attr = [] ) {
		...
		echo '<span>';
			...
			echo '<input',
				...
				// phpcs:ignore WordPress.Security.EscapeOutput -- Reason: $disabled_attribute is generated by WordPress.
				$disabled_attribute,
				' /> ';
			echo '<input',

...
<div id="home" class="wpseotab">
	...
	if ( 'posts' == get_option( 'show_on_front' ) ) {
		...
		echo '<p>', sprintf( __( 'You can determine the title and description for the front page by %sediting the front page itself &raquo;%s', 'wordpress-seo' ), sprintf( '<a href="%s">', esc_url( get_edit_post_link( get_option( 'page_on_front' ) ) ) ), '</a>' ), '</p>';
		if ( get_option( 'page_for_posts' ) > 0 ) {
			echo '<p>', sprintf( __( 'You can determine the title and description for the blog page by %sediting the blog page itself &raquo;%s', 'wordpress-seo' ), sprintf( '<a href="%s">', esc_url( get_edit_post_link( get_option( 'page_for_posts' ) ) ) ), '</a>' ), '</p>';
		}
	}
	?>
</div>
	...
	if ( is_array( $post_types ) && $post_types !== array() ) {
		foreach ( $post_types as $pt ) {
			...
			if ( $warn === true ) {
				...
				echo '<p class="error-message">', sprintf( __( 'So remember: If you change the %sattachment redirection setting%s in the future, the below settings will take effect for *all* media items.', 'wordpress-seo' ), sprintf( '<a href="%s">', esc_url( admin_url( 'admin.php?page=wpseo_permalinks' ) ) ), '</a>' ), '</p>';
			}

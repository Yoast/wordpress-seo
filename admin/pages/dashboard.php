...
if ( is_array( $options['blocking_files'] ) && count( $options['blocking_files'] ) > 0 ) {
	echo '
		<p id="blocking_files" class="wrong">
...
if ( $options['theme_description_found'] !== '' ) {
	echo '
		<p id="metadesc_found notice" class="wrong settings_error">
			...
			<code>', esc_html( $options['theme_description_found'] ), '</code>
		</p>';
}
...
if ( strpos( get_option( 'permalink_structure' ), '%postname%' ) === false && $options['ignore_permalink'] === false ) {
	echo '
		<p id="wrong_permalink" class="wrong">
			...
			__( 'You do not have your postname in the URL of your posts and pages, it is highly recommended that you do. Consider setting your permalink structure to <strong>/%postname%/</strong>.', 'wordpress-seo' ), '
		</p>';
}

if ( get_option( 'page_comments' ) && $options['ignore_page_comments'] === false ) {
	echo '
		<p id="wrong_page_comments" class="wrong">
			...
			__( 'Paging comments is enabled, this is not needed in 999 out of 1000 cases, so the suggestion is to disable it, to do that, simply uncheck the box before "Break comments into pages..."', 'wordpress-seo' ), '
		</p>';
}

echo '
		<h2>', __( 'General', 'wordpress-seo' ), '</h2>';

if ( $options['ignore_tour'] === true ) {
	echo '
		<label class="select">', __( 'Introduction Tour:', 'wordpress-seo' ), '</label><a class="button-secondary" href="', esc_url( admin_url( 'admin.php?page=wpseo_dashboard&wpseo_restart_tour=1' ) ), '">', __( 'Start Tour', 'wordpress-seo' ), '</a>
		<p class="desc label">', __( 'Take this tour to quickly learn about the use of this plugin.', 'wordpress-seo' ), '</p>';
}

echo '
		<label class="select">', __( 'Default Settings:', 'wordpress-seo' ), '</label><a onclick="if( !confirm(\'' . __( 'Are you sure you want to reset your SEO settings?', 'wordpress-seo' ), '\') ) return false;" class="button-secondary" href="', esc_url( add_query_arg( array( 'nonce' => wp_create_nonce( 'wpseo_reset_defaults' ) ), admin_url( 'admin.php?page=wpseo_dashboard&wpseo_reset_defaults=1' ) ) ), '">', __( 'Reset Default Settings', 'wordpress-seo' ), '</a>
		<p class="desc label">', __( 'If you want to restore a site to the default WordPress SEO settings, press this button.', 'wordpress-seo' ), '</p>';

echo '
		<h2>', __( 'Tracking', 'wordpress-seo' ), '</h2>
		', $wpseo_admin_pages->checkbox( 'yoast_tracking', __( 'Allow tracking of this WordPress install\'s anonymous data.', 'wordpress-seo' ) ),
		'<p class="desc">', __( "To maintain a plugin as big as WordPress SEO, we need to know what we're dealing with: what kinds of other plugins our users are using, what themes, etc. Please allow us to track that data from your install. It will not track <em>any</em> user details, so your security and privacy are safe with us.", 'wordpress-seo' ), '</p>

		<h2>', __( 'Security', 'wordpress-seo' ), '</h2>
		', $wpseo_admin_pages->checkbox( 'disableadvanced_meta', __( 'Disable the Advanced part of the WordPress SEO meta box', 'wordpress-seo' ) ),
		'<p class="desc">', __( 'Unchecking this box allows authors and editors to redirect posts, noindex them and do other things you might not want if you don\'t trust your authors.', 'wordpress-seo' ), '</p>

		<h2>', __( 'Webmaster Tools', 'wordpress-seo' ), '</h2>
		<p>', __( 'You can use the boxes below to verify with the different Webmaster Tools, if your site is already verified, you can just forget about these. Enter the verify meta values for:', 'wordpress-seo' ), '</p>
		...
		', $wpseo_admin_pages->textinput( 'pinterestverify', '<a target="_blank" href="https://help.pinterest.com/entries/22488487-Verify-with-HTML-meta-tags">' . __( 'Pinterest', 'wordpress-seo' ) . '</a>' ), '

...
class WPSEO_Bulk_Title_Editor_List_Table extends WP_List_Table {
	...
	function get_views() {
		...
		$status_links['all'] = "<a href='admin.php?page=wpseo_bulk-title-editor'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts' ), number_format_i18n( $total_posts ) ) . '</a>';
		...
		foreach( get_post_stati( array('show_in_admin_all_list' => true), 'objects' ) as $status ) {
			...
			$status_links[ $status_name ] = "<a href='admin.php?page=wpseo_bulk-title-editor&amp;post_status=$status_name'$class>" . sprintf( translate_nooped_plural( $status->label_count, $total ), number_format_i18n( $total ) ) . '</a>';
		...
		$status_links[ 'trash' ] = "<a href='admin.php?page=wpseo_bulk-title-editor&amp;post_status=trash'$class>" . sprintf( _nx( 'Trash <span class="count">(%s)</span>', 'Trash <span class="count">(%s)</span>', $trashed_posts, 'posts' ), number_format_i18n( $trashed_posts ) ) . '</a>';

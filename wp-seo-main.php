 ...
 *
 * @param int $blog_id The id of the blog for which to run the activation actions
 */
...
function yoast_wpseo_self_deactivate() {
	if ( is_admin() ) {
		...
		add_action( 'admin_notices', create_function( $message, 'echo sprintf( \'<div class="error"><p>%s %s</p></div>\', __( \'Activation failed:\', \'wordpress-seo\' ), $message );' ) );

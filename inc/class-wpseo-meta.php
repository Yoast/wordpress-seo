...
class WPSEO_Meta {
	 ...
	 * @static
	 *
	 * @param  boolean $value      The current status to allow updating metadata for the given type.
	 ...
	 *
	 * @return bool|null           true = stop saving, false|null = continue saving
	 */
	public static function remove_meta_if_default( $value, $object_id, $meta_key, $meta_value, $prev_value = '' ) {
		...
		return $value; // Go on with the normal execution (update) in meta.php.
	}
	 ...
	 * @static
	 *
	 * @param  boolean $value      The current status to allow adding metadata for the given type.
	 ...
	 * @return bool|null           true = stop saving, false|null = continue saving
	 */
	public static function dont_save_meta_if_default( $value, $object_id, $meta_key, $meta_value ) {
		/* If it's one of our meta fields, check against default */
		if ( isset( self::$fields_index[ $meta_key ] ) && self::meta_value_is_default( $meta_key, $meta_value ) === true ) {
			return true; // Stop saving the value.
		}

		return $value; // Go on with the normal execution (add) in meta.php.
	}

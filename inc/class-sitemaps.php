...
if ( ! class_exists( 'WPSEO_Sitemaps' ) ) {
	...
	class WPSEO_Sitemaps {
		...
		function build_post_type_map( $post_type ) {
			...
			// add additional post statuses
			// the result will be stored inside an IN query.
			// ex. return:, 'publish','internal-review','super-draft'
			$status = apply_filters('wpseo_posts_post_status', $status, $post_type);
			...
			while ( $total > $offset ) {
				...
				$query = $wpdb->prepare( "SELECT l.ID, post_title, post_content, post_name, post_author, post_parent, post_modified_gmt, post_date, post_date_gmt FROM ( SELECT ID FROM $wpdb->posts {$join_filter} WHERE post_status IN ('%s') AND post_password = '' AND post_type = '%s' AND post_author != 0 AND post_date != '0000-00-00 00:00:00' {$where_filter} ORDER BY post_modified ASC LIMIT %d OFFSET %d ) o JOIN $wpdb->posts l ON l.ID = o.ID ORDER BY l.ID",
